...
export default function TechStack() {
    ...
    const skills = [{
        ...
        name: "Backend",
        skill: ["Node.js", "Express", "Nest.js", "Hono", "FastAPI", "PostgreSQL", "PineCone", "Prisma", "MongoDB"]
    }, {
        name: "Others",
    ...
    return <div className={`${theme != "light" ? "bg-bgds text-white" : "bg-white text-bgds"} mx-auto select-none `}>
        ...
        <div className={`flex flex-wrap gap-4 justify-center mb-32`}>
            {techs.map((tech) => (
                <div className={`group transition-all ease-in-out duration-500 w-24 h-24 border-2  ${theme != "light" ? "border-brdr hover:bg-forHover" : "border-red-300 hover:bg-red-300"} flex items-center justify-center rounded-lg `}>
        ...
        <div className="flex flex-wrap gap-8 p-10 font-chill  justify-center ">

            {
                skills.map((item, index) => (
                    <div key={index} className={`border-2 w-[450px] h-[350px]   p-16 rounded-lg transition-all ease-in-out duration-500 ${theme != "light" ? "bg-bgds border-brdr hover:bg-forHover" : "bg-white border border-red-300 hover:bg-red-300"} `}>
